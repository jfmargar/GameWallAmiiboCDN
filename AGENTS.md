...
- Mantén el flujo `Backlog → Ready → In progress → In review → Done`: pasa a `In progress` al asignar/crear rama, a `In review` tras validar, subir y abrir PR, y a `Done` solo tras merge y cierre.
